...
import { UploadOutlined, RotateRightOutlined, DownloadOutlined, ZoomInOutlined, ZoomOutOutlined } from "@ant-design/icons";
          ...
          <div style={{ width: "100%", maxWidth: 400, margin: "20px auto", position: "relative" }}>
            <ReactCrop src={selectedImage} crop={crop} onChange={setCrop} onComplete={handleCropComplete}>
              <img
                ...
                src={selectedImage}
                alt="Crop Preview"
                style={{ transform: `rotate(${rotation}deg) scale(${zoom})`, maxWidth: "100%" }}
              />
            </ReactCrop>
          </div>
          <Space>
            <Button onClick={handleRotate} icon={<RotateRightOutlined />}>Rotate 90°</Button>
            <Button onClick={handleZoomIn} icon={<ZoomInOutlined />}>Zoom In</Button>
            <Button onClick={handleZoomOut} icon={<ZoomOutOutlined />}>Zoom Out</Button>
  ...
  return new Promise((resolve) => {
    canvas.toBlob((blob) => {
      if (blob) {
        ...
        message.error("Failed to create cropped image.");
        resolve(null);
      }
    }, "image/png");
  });
